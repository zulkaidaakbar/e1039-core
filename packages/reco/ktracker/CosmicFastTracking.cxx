/*
CosmicFastTracking.cxx

Implementation of class Tracklet, CosmicFastTracking
...
CosmicFastTracking::CosmicFastTracking(const PHField* field, const TGeoManager* geom, bool flag): verbosity(0), enable_KF(flag)
{
    using namespace std;

#ifdef _DEBUG_ON
    cout << "Initialization of CosmicFastTracking ..." << endl;
    cout << "========================================" << endl;
#endif
...
CosmicFastTracking::~CosmicFastTracking()
{
...
void CosmicFastTracking::setRawEventDebug(SRawEvent* event_input)
{
    rawEvent = event_input;
    hitAll = event_input->getAllHits();
}

int CosmicFastTracking::setRawEvent(SRawEvent* event_input)
{
	//reset timer
...
bool CosmicFastTracking::acceptEvent(SRawEvent* rawEvent)
{
    ...
    if(rawEvent->getNHitsInD2() > 25) return false; // 23% - 28 hit per plane
    if(rawEvent->getNHitsInD3p() > 25) return false;// 18% - 23 hit per plane
    if(rawEvent->getNHitsInD3m() > 25) return false;// 18% - 23 hit per plane
...
void CosmicFastTracking::buildBackPartialTracks()
{
#ifndef ALIGNMENT_MODE
    ...
    for(std::list<Tracklet>::iterator tracklet3 = trackletsInSt[2].begin(); tracklet3 != trackletsInSt[2].end(); ++tracklet3)
    {
        ...
        {
           // if(fabs(tracklet2->tx - tracklet3->tx) > 0.15 || fabs(tracklet2->ty - tracklet3->ty) > 0.1) continue; //disable for cosmic reco
...
void CosmicFastTracking::buildGlobalTracks()
{
...
void CosmicFastTracking::resolveLeftRight(Tracklet& tracklet, double threshold)
{
#ifdef _DEBUG_ON
...
void CosmicFastTracking::resolveSingleLeftRight(Tracklet& tracklet)
{
#ifdef _DEBUG_ON
...
void CosmicFastTracking::removeBadHits(Tracklet& tracklet)
{
#ifdef _DEBUG_ON
...
void CosmicFastTracking::resolveLeftRight(SRawEvent::hit_pair hpair, int& LR1, int& LR2)
{
    LR1 = 0;
    LR2 = 0;
...
void CosmicFastTracking::buildTrackletsInStation(int stationID, int listID, double* pos_exp, double* window)
{
#ifdef _DEBUG_ON
...
bool CosmicFastTracking::acceptTracklet(Tracklet& tracklet)
{
   ...
   // if(!hodoMask(tracklet)) return false; //disable for cosmic reco

    //For back partials, require projection inside KMAG, and muon id in prop. tubes
    if(tracklet.stationID > nStations-2)
    {
       // if(!p_geomSvc->isInKMAG(tracklet.getExpPositionX(Z_KMAG_BEND), tracklet.getExpPositionY(Z_KMAG_BEND))) return false; disable for cosmic reco
#ifndef ALIGNMENT_MODE
...
bool CosmicFastTracking::hodoMask(Tracklet& tracklet)
{
...
bool CosmicFastTracking::muonID_search(Tracklet& tracklet)
{
    ...
    if(segs[0]->getNHits() + segs[1]->getNHits() >= /*5*/1) //change for cosmic reco
    {
        return true;
    }
    else if(segs[1]->getNHits() == 1 || segs[1]->getNPlanes() == 1)
    {
        return segs[1]->nHodoHits >= /*2*/1; //change for cosmic reco
    }
    return false;
}

bool CosmicFastTracking::muonID_comp(Tracklet& tracklet)
{
    //Set the cut value on multiple scattering
    //multiple scattering: sigma = 0.0136*sqrt(L/L0)*(1. + 0.038*ln(L/L0))/P, L = 1m, L0 = 1.76cm
    double cut = 0.03;
    ...
    if(segs[0]->getNHits() + segs[1]->getNHits() < /*5*/1) return false;
    return true;
}

bool CosmicFastTracking::muonID_hodoAid(Tracklet& tracklet)
{
    double win = 0.03;
    double factor = 5.;
    if(tracklet.stationID == nStations)
...
void CosmicFastTracking::buildPropSegments()
{
#ifdef _DEBUG_ON
...
int CosmicFastTracking::fitTracklet(Tracklet& tracklet)
{
...
int CosmicFastTracking::reduceTrackletList(std::list<Tracklet>& tracklets)
{
...
void CosmicFastTracking::getExtrapoWindowsInSt1(Tracklet& tracklet, double* pos_exp, double* window, int st1ID)
{
...
void CosmicFastTracking::getSagittaWindowsInSt1(Tracklet& tracklet, double* pos_exp, double* window, int st1ID)
{
    if(tracklet.stationID != nStations-1)
    {
        for(int i = 0; i < 3; i++)
...
void CosmicFastTracking::printAtDetectorBack(int stationID, std::string outputFileName)
{
    TCanvas c1;
...
SRecTrack CosmicFastTracking::processOneTracklet(Tracklet& tracklet)
{
    //tracklet.print();
    KalmanTrack kmtrk;
...
bool CosmicFastTracking::fitTrack(KalmanTrack& kmtrk)
{
...
void CosmicFastTracking::resolveLeftRight(KalmanTrack& kmtrk)
{
    bool isUpdated = false;
...
void CosmicFastTracking::printTimers() {
	std::cout <<"CosmicFastTracking::printTimers: " << std::endl;
	std::cout <<"================================================================" << std::endl;
...
void CosmicFastTracking::chi2fit(int n, double x[], double y[], double& a, double& b)
{
    double sum = 0.;
    double sx = 0.;
    double sy = 0.;
